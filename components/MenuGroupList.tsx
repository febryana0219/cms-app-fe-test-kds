...
export default function MenuGroupList() {
  const menuGroups = useStore((s) => s.menuGroups as MenuGroup[]);
  const menus = useStore((s) => s.menus as Menu[]);
  ...
  const [editingGroup, setEditingGroup] = React.useState<MenuGroup | null>(null);
  ...
  return (
    <Box>
            ...
            <CardContent>
              <Stack direction="row" justifyContent="space-between" alignItems="center">
                ...
                <div>
                  ...
                  <IconButton onClick={() => deleteMenuGroup(g.id)}>
                    <DeleteIcon />
                  </IconButton>
                  <Button
                    startIcon={<AddIcon />}
                    onClick={() => {
                      setMenuGroupForAdd(g.id);
                      setEditingMenu(null);
              ...
              <List>
                {menus.filter((m) => m.groupId === g.id).length === 0 && (
                  <ListItem>
                    <ListItemText primary="No menus in this group." />
                  </ListItem>
                )}
      ...
      <MenuForm
        open={menuFormOpen}
        groupId={menuGroupForAdd}
        initial={editingMenu || undefined}
        onClose={() => {
          setMenuFormOpen(false);
          setMenuGroupForAdd(undefined);
          setEditingMenu(null);
        }}
      />
    </Box>
